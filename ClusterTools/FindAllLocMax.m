...
% maxima as VOIs, using LocMaxClusters2NIFTI.m, see help there.
%
% The iteration continues till the number of local maxima (given minimum distance)
% does not change any more for three iterations (this can be changed via additonal inputs).
%
%Inputs:
...
%                   .LocMaxStats         (NClustersx3)   The first column is the statistics value at the local maxima
%                                                        and the second column is the average statistics values of the cluster,
%                                                        and the third column is the standard deviation of the stats values of the cluster.
%                   .LocMaxClusterSize   (NClustersx1)   The cluster size for each cluster. 
%                   .LocMaxMinDist_mm    (1x1)           Distance in mm that the local maxima should (AT LEAST!) be apart.
...
%Comment V1.61: (03.12.2016): Update description a little bit. (LocMaxStats has local maxima the mean stats val and the stdev of stats vals per cluster) V1.6: (22.03.2016): NIterMax now enforce that it should be 20 at a minimum. V1.5: (21.02.2016) add cluster size for each local maximum as extra information AT THE END. V1.4: (24.10.2015): CoG-Coords "LocMaxCoGCoords" are a problem: just fixed by force here, see below, need to redesign this whole thing regarding this, see below. V1.3: (30.04.2015) Use Sign of StatsVals to enable unified search for local MAXIMA and use Sign later to change stats vals back after finishing iterations. V1.2: (31.01.2015): Bug-fix "LocMaxStruct.LocMaxStats" had identical columns before, but that is fixed now. V1.1: (25.12.2014): initial implementation (changed version from test script FindAllLocMax.m)
...
%% determine SIGN of StatsVals such that we can use this later to give the right sign to the results, given that this tool only works on maxima.
AllSigns = sign(StatsVals); %all signs
if(all(AllSigns>0))
    SignStatsVals = 1;
else
    if(all(AllSigns<0))
        ...
        error('FindAllLocMax>IterateFindLocMax: StatsVals has negative & positive values! Only one sign is allowed. Clustering should be separately done for negative and positive StatsVals!');
    end
end
StatsVals = abs(StatsVals); %make StatsVals positive AND use maximum later for finding local maximum.    
...
%there can be a problem for small number of voxels therefore check the minimum to be at least 20!
if(NIterMax<20)
    NIterMax = 20;
end
...
while(KeepIterating)
    ...
    [ClusterNr_LocMax,Coords_LocMax,StatsVals_LocMax,DistMat_LocMax,LocMaxCoords,LocMaxCoGCoords] = IterateFindLocMax(Coords_LocMax,StatsVals,LocMaxMinDist_mm,ConnectMat,Verbose);
    ...
    LocMaxStruct.Iterations.StatsVals_LocMax{KeepIterating,1} = SignStatsVals.*StatsVals_LocMax; %apply sign
    ...
    if(rSSD==0)
        if(~ConvergenceOccured) %make sure to only save the result at the convergence point, in case we have odd behavior later for some reason.
            %BugFix 24.10.2015 --> this needs to be redesigned, i.e. not calculated all the time in IterateFindLocMax.m because it only works with the end result after having the cluster assignments, otherwise it will just result in the same values as LocMaxCoords.
            LocMaxClusterSize = zeros(max(ClusterNr_LocMax(:)),1); %add on LocMaxClusterSize 21.02.2016
            ...
            for IndCluster = 1:max(ClusterNr_LocMax(:))
                LocMaxCoGCoords(IndCluster,:) = mean(Coords_mm(ClusterNr_LocMax==IndCluster,:),1); %average coordinate of the cluster of the local maxima
                LocMaxClusterSize(IndCluster) = length(find(ClusterNr_LocMax==IndCluster)); %number of voxels for current cluster.
            end
            %END-BugFix & %END-add on LocMaxClusterSize 21.02.2016
            ...
            LocMaxStruct.LocMaxClusterSize  = LocMaxClusterSize;%(NClustersx1)   The number of voxels belonging to each cluster associated with each local maxima.
            ...
            LocMaxStruct.LocMaxStats        = SignStatsVals.*GetLocMax_n_ClusterAverage_StatsVals(StatsVals,ClusterNr_LocMax); %apply sign %OLD: LocMaxStatsVals;  %(NClustersx3)   The first column is the statistics value at the local maxima and the second column is the average statistics values of the cluster and the third is the stdev.
    ...
    if(KeepIterating>NIterMax)
        error(['ERROR! NIterMax(',num2str(NIterMax),') reached. Something is strange.']);
    end
end
t_iterations = toc;
...
function LocMaxStatsVals_n_ClusterAverageClusterStdev = GetLocMax_n_ClusterAverage_StatsVals(StatsVals,ClusterNr_LocMax)
...
LocMaxStatsVals_n_ClusterAverageClusterStdev = zeros(length(Clusters),3);
...
for IndCluster = 1:length(Clusters)
    ...
    LocMaxStatsVals_n_ClusterAverageClusterStdev(IndCluster,1) = max( CurrStatsVals(:));
    LocMaxStatsVals_n_ClusterAverageClusterStdev(IndCluster,2) = mean(CurrStatsVals(:));
    LocMaxStatsVals_n_ClusterAverageClusterStdev(IndCluster,3) = std( CurrStatsVals(:));
end
