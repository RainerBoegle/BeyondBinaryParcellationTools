...
%      [OutputPath,Vo] = LocMaxClusters2NIFTI(LocMaxStruct.ClusterNr_vox,MapExtractStruct.Negative|Positive.Coords_vox,MapExtractStruct.V_map); %if using ClusterStatsMap results %manual selection of OutputPath(+FileName) 
%
%V1.0
...
try %TemplateVol
    ...
    disp_catch(CATCH_TemplateVol,mfilename,'CATCH_TemplateVol');
...
try %OutputPath
    ...
    disp_catch(CATCH_OutputPath,mfilename,'CATCH_OutputPath');
    FilterIndex = 0;
    while(~FilterIndex)
        [OutputPath_FileName,OutputPath_Dir,FilterIndex] = uiputfile('.nii','Save Cluster-NIFTI...','Clusters.nii');
    end
