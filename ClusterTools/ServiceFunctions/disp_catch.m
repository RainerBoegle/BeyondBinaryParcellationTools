...
%Comment V2.0: (28.02.2015): initial implementation (changed version from original subfunction of various display tools functions)
...
if(~isempty(CATCHobj.stack))
    for IndStack = 1:length(CATCHobj.stack)
        disp(['Stack ',num2str(IndStack),': Error in "',CATCHobj.stack(2).name,'" at line ',num2str(CATCHobj.stack(IndStack).line),'. (',CATCHobj.stack(IndStack).file,')']);
    end
end
