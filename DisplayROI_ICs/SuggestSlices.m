...
%                     For each image find the maximum statistics value (keep that slice) and remove
...
%
%Usage:
%       [SuggestedSlices] = SuggestSlices(NIFTI_files,Mode,varargin);
%
%V2.2
%Author: Rainer Boegle (Rainer.Boegle@googlemail.com)
%Comment V2.2: (21.11.2015): Allow thresholds to be entered as well. V2.1: (13.02.2015): 'Cluster'-mode: if only one value per input, i.e. [0,1] mask --> pick more slices. V2.0(28.02.2015): initial implementation (changed version from original subfunction of various display tools functions)

%% Defaults
MaxNumSlices = 42;

%% NIFTI_files has to be a cellstring
    ...
    for Ind = 1:length(CommStrings)
        switch(CommStrings{Ind})
            ...
            case {'Thresh','Threshold'}
                if(isnumeric(CommVals{Ind}))
                    Thresholds = CommVals{Ind};
                else
                    error('Thresholds Command-Value must be numberic!');
                end
            otherwise
...
switch(Mode)
    ...
    case {'Stats','stats','Statistics','statistics'}
        try
            [SuggestedSlices_max,Res_z] = SuggestSlicesForStatsImages(NIFTI_files,'max',Thresholds);
             SuggestedSlices_min        = SuggestSlicesForStatsImages(NIFTI_files,'min',Thresholds);
...
function [SuggestedSlices,Res_z] = SuggestSlicesForStatsImages(NIFTI_files,StatsMode,Thresholds)
...
for IndData = 1:length(NIFTI_files)
    ...
    if(~isempty(Thresholds))
        if(~isnan(Thresholds(IndData,1)))
            Data(Data<0&Data>Thresholds(IndData,1)) = 0;
        end
        if(~isnan(Thresholds(IndData,2)))
            Data(Data>0&Data<Thresholds(IndData,2)) = 0;
        end
    end
    v2m     = NII_tmp.mat;
